...
def lambda_handler(event, context):
    ...
    for item in event['detail']['additional-information']['environment']['environment-variables']:
        if item['name'] == 'PULL_REQUEST_ID': pull_request_id = item['value']
        if item['name'] == 'REPOSITORY_NAME': repository_name = item['value']
        if item['name'] == 'SOURCE_COMMIT': before_commit_id = item['value']
        if item['name'] == 'DESTINATION_COMMIT': after_commit_id = item['value']
